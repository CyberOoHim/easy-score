...
export const HumToScoreModal: React.FC<HumToScoreModalProps> = ({
  ...
  const content = (
    <div
      id={isEmbedded ? "hum-to-score-studio-container" : "hum-to-score-modal-dialog"}
      className={isEmbedded 
        ? "w-full bg-white dark:bg-[#12151c] border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-xl overflow-hidden flex flex-col transition-all"
        : "w-full max-w-3xl bg-white dark:bg-[#12151c] border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col my-auto transition-all"
      }
    >
      {/* MODAL / STUDIO HEADER */}
      <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-200/80 dark:border-zinc-800/80 bg-zinc-50/70 dark:bg-[#161a24]/70">
        ...
        <button
          ...
          title={isEmbedded ? "重設回到設定" : "關閉"}
        >
          {isEmbedded ? <RotateCcw className="w-5 h-5" /> : <X className="w-5 h-5" />}
        </button>
      </div>

      {/* MODAL / STUDIO BODY */}
      <div className="p-5 sm:p-6 flex flex-col gap-5 max-h-[85vh] overflow-y-auto">
          {/* Error Banner */}
          {audioError && (
            <div
